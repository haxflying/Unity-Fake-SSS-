	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float3 sss = _BaseColor * (1 - thickness) * _SSS_Scale;
				...
				half4 color = half4(sss * max(_LightColor0, ambient) + (wrap_diffuse * max(_LightColor0.rgb, ambient) + ambient/*gi.indirect.diffuse*/) * _BaseColor
